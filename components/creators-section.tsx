...
export default function CreatorsSection({ lang }: { lang: Lang }) {
  ...
  return <section className="home-creators" dir={rtl ? 'rtl' : 'ltr'} aria-labelledby="home-creators-heading">
    ...
    <div className="homepage-creators-grid" aria-label={rtl ? 'قائمة صنّاع المحتوى' : 'Content creator list'}>
      {creators.slice(0, 8).map((creator) => <HomepageCreatorCard key={creator.id} creator={creator} lang={lang} />)}
    </div>
    <Link className="creator-showcase-view-all" href="/creators">{rtl ? 'عرض جميع صناع المحتوى' : 'VIEW ALL CREATORS'}<ArrowUpRight size={16} /></Link>
  </section>;
}
