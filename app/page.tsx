...
const copy = {
  ar: {
    ...
    newAccount: 'ليس لديك حساب؟ أنشئ حساباً',
    ...
    open: 'فترة الفحص مفتوحة',
    beforeAfter: 'قبل / بعد / الفرق',
    approved: 'معتمد',
    pending: 'بانتظار',
    rejected: 'مرفوض',
    approve: 'اعتماد',
    ...
    export: 'تصدير CSV',
    deleteAsk:
      'هل تريد حذف اللاعب وكل سجلاته نهائياً؟',
    ...
    error: 'تحقق من البيانات وحاول مجدداً.',
    duel: 'استعد للقتال',
    adminDuel: 'فتح غرفة القيادة',
    language: 'English',
  },
  ...
  en: {
    ...
    newAccount: 'New here? Create your account',
    ...
    online: 'Online now',
    utc: 'UTC time',
    open: 'Scan period is open',
    beforeAfter: 'Before / after / change',
    approved: 'Approved',
    pending: 'Pending',
    rejected: 'Rejected',
    approve: 'Approve',
    ...
    export: 'Export CSV',
    deleteAsk:
      'Delete this player and every record permanently?',
    ...
    duel: 'Prepare for battle',
    adminDuel: 'Opening command room',
    language: 'العربية',
  },
} as const;
 ...
 const saveAdmin=async(e:FormEvent)=>{
  ...
  await fetch('/api/submissions',{
    ...
    body:JSON.stringify({
      ...
      defeat:Number(draft.defeat),
      troops:Number(draft.troops),
      note:draft.note
    })
  });
...
const exportCsv=()=>{
  const rows=[
    ['Player ID','Name','Power','Kills','Defeat','Troops','Status'],
    ...scans.map(s=>[
      s.playerId,
      s.playerName,
      s.power,
...
const totals=useMemo(
  ()=>scans
    ...
    .reduce(
      (x,s)=>({
        power:x.power+s.power,
        kills:x.kills+s.kills,
        losses:x.losses+s.defeat
      }),
      {power:0,kills:0,losses:0}
    ),
  [scans]
);
...
const Auth=gate&&(
  <div className="gate-backdrop">
    <form
      className={`gate-card blade-auth emboss-auth ${
        gate==='admin'?'gate-admin':''
      } ${
        gate==='player'&&authMode==='login'
          ?'gate-login'
          :'gate-register'
      }`}
      onSubmit={authSubmit}
    >
      ...
      >
        ...
        <div className="emboss-lock">
          {gate==='admin'?<Shield/>:<LockKeyhole/>}
        </div>

        <p className="emboss-kicker">
          RK75 / {gate==='admin'?'COMMAND':'FIGHTER ACCESS'}
        </p>

        <h2 className="welcome-title">
          {gate==='admin'
            ?(rtl?'دخول القائد':'Commander access')
            :authMode==='register'
              ?(rtl?'أنشئ حسابك':'Create account')
              :(rtl?'مرحبًا بعودتك':'Welcome back')
          }
        </h2>

        <span className="emboss-subtitle">
          {gate==='admin'
            ?(rtl
              ?'منطقة إدارة التحالف المحمية.'
              :'Protected alliance management area.')
            :(rtl
              ?'استخدم معرّف اللاعب وكلمة المرور الخاصة بك.'
              :'Use your player ID and your private password.')
          }
        </span>

        <div className="emboss-fields">

          {gate==='player'&&(
            <label className="emboss-field">
              <span>{t.id}</span>
              ...
              <input
                ...
                onChange={e=>
                  setAuth({
                    ...auth,
                    playerId:e.target.value.replace(/\D/g,'')
                  })
                }
              />
            </label>
          ...
          <label className="emboss-field password-field">
            <span>
              {gate==='admin'?t.adminPass:t.pass}
            </span>
            ...
            <div>
              ...
              <button
                ...
                aria-label={
                  showPassword
                    ?(rtl?'إخفاء كلمة المرور':'Hide password')
                    :(rtl?'إظهار كلمة المرور':'Show password')
                }
        ...
        {gate==='player'&&(
          <label className="emboss-remember">
            <input
              type="checkbox"
              ...
              onClick={()=>{
                ...
                if(next==='login'){
                  setAuth({
                    ...
                    playerId:
                      auth.playerId||
                      localStorage.getItem('rk75-last-player-id')||
                      ''
                  });
                }
              }}
...
const Entrance=entrance&&(
  <div
    className={`system-entry blade-entry ${
      entrance==='admin'
        ?'system-entry-admin'
        :''
    }`}
  ...
  >
    ...
    <section className="command-stage">
      ...
      <div className="stage-copy">
        ...
        <p>
          {rtl
            ?'نظام فخم لمتابعة القوة والقتل والخسائر والجنود لكل عضو، قبل وبعد كل فحص.'
            :'A premium system to track power, kills, losses, and troops for every member — before and after every scan.'
          }
        </p>
    ...
    <section className="home-creators">
      ...
      <div className="home-creators-head">
        ...
        <div>
          ...
          <span>
            القنوات المحفوظة رسميًا في RK75.
          </span>
        </div>
...
if(view==='player')return(
  ...
  >
    ...
    <section className="fighter-hero">
      ...
      <p>RK75 / PLAYER ACCOUNT</p>

      <h1>{t.welcome}</h1>

      <span>{t.accountHint}</span>
    ...
    <section className="fighter-grid">
      ...
      >
        ...
        <button
          className="ember-button form-submit"
          disabled={busy}
        >
          {playerEdit
            ?t.save
  ...
  >
    ...
    <section className="admin-kpis">
      ...
      <Kpi
        icon={<Target/>}
        label={t.power}
        value={n(totals.power)}
      />

      <Kpi
        icon={<Swords/>}
        label={t.kills}
        value={n(totals.kills)}
      />

      <Kpi
        icon={<Users/>}
        label={t.losses}
        value={n(totals.losses)}
      />

      <Kpi
        icon={<Activity/>}
        label={t.online}
        value={String(presence.online)}
      />
...
function StatsInputs({
  ...
  draft: StatsDraft;
  setDraft: (v: StatsDraft) => void;
  t: any;
}) {
  const field = (
    key: keyof StatsDraft,
    label: string,
    kind: 'text' | 'number' = 'number'
  ) => (
    <label>
      {label}
  ...
  return (
    <>
      <p className="comparison-form-hint">
        {t.comparisonHint}
      </p>
...
const encryptVault = async (
  ...
  output.set(iv);
  output.set(encryptedData, iv.length);
...
const decryptVault = async (
  ...
  const encryptedData = base64ToBytes(value);
...
function VaultPanel({
  ...
  const c = rtl
    ? {
        title: 'الخزنة الخاصة',
        open: 'فتح الخزنة',
        setup: 'أنشئ رمز الخزنة الخاص بك',
        unlock: 'افتح خزنتك',
        desc: 'البريد وكلمات المرور تُشفّر في جهازك قبل حفظها. لا يمكن فتحها دون رمز الخزنة.',
        pass: 'رمز الخزنة',
        confirm: 'تأكيد رمز الخزنة',
        label: 'اسم الحساب',
        email: 'البريد الإلكتروني',
        password: 'كلمة المرور',
        add: 'حفظ الحساب',
        empty: 'لا توجد حسابات محفوظة.',
        wrong: 'رمز الخزنة غير صحيح.',
        mismatch: 'الرمزان غير متطابقين أو قصيران.',
        close: 'إغلاق',
        hide: 'إخفاء',
        show: 'إظهار',
      }
    : {
        title: 'Private vault',
        open: 'Open vault',
        setup: 'Create your private vault passphrase',
        unlock: 'Unlock your vault',
        desc: 'Emails and passwords are encrypted in your browser before they are saved. The vault cannot open without its passphrase.',
        pass: 'Vault passphrase',
        ...
        wrong: 'The vault passphrase is incorrect.',
        mismatch: 'Passphrases do not match or are too short.',
        close: 'Close',
        hide: 'Hide',
        show: 'Show',
      };
  ...
  const [settings, setSettings] = useState<
    { salt: string; verifier: string } | null | undefined
  >(undefined);
  ...
  const unlock = async () => {
    ...
    try {
      ...
      if (settings) {
        activeKey = await vaultKey(
          passphrase,
          settings.salt
        );

        const verifier = await decryptVault(
          activeKey,
          settings.verifier
        );

        if (verifier !== 'RK75_VAULT_OK') {
          throw new Error('bad');
        }
      } else {
        ...
        const verifier = await encryptVault(
          activeKey,
          'RK75_VAULT_OK'
        );
  ...
  const add = async () => {
    ...
    try {
      const encryptedPayload = await encryptVault(
        key,
  ...
  return (
    <>
      <button
        className="outline-button vault-trigger"
        onClick={openPanel}
      >
        <LockKeyhole size={16} />
        {c.title}
      ...
      {open && (
        <div className="gate-backdrop">
          <div className="gate-card vault-card">
