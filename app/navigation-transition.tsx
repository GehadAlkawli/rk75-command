...
const routePaths = new Set(['/', '/stats', '/accounts']);

export default function NavigationTransition() {
  useEffect(() => {
    const markCurrentRoute = () => {
      const current = window.location.pathname;
      document.querySelectorAll<HTMLAnchorElement>('.neo-top a[href], .market-nav a[href], .comparison-nav a[href]').forEach((link) => {
