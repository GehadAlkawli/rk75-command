...
export default function LivePage() {
  ...
  return <main className="creator-page" dir={rtl ? 'rtl' : 'ltr'}><header className="media-nav"><a className="neo-brand" href="/"><span>RK</span><b>75</b><i>LIVE</i></a><HamburgerMenu label={rtl ? 'القائمة' : 'Menu'} locale={rtl ? 'ar' : 'en'} direction={rtl ? 'rtl' : 'ltr'}><a href="/creators">{t.creators}</a><a href="/media">{t.media}</a><button className="lang-switch" onClick={() => setLang(rtl ? 'en' : 'ar')}>{t.lang}</button><a className="stats-back" href="/"><ArrowLeft size={16}/>{t.home}</a></HamburgerMenu></header><section className="creator-hero"><p><Radio size={15}/>{t.tag}</p><h1>{t.title}</h1><span>{t.sub}</span></section><section className="creator-grid">{loading ? <div className="creator-loading"><LoaderCircle className="spin"/>{t.loading}</div> : creators.map((creator) => <CreatorCard key={creator.id} creator={creator} lang={lang}/>)}</section>{!loading && !creators.length && <section className="creator-empty"><Radio/><h2>{t.empty}</h2></section>}</main>;
}
